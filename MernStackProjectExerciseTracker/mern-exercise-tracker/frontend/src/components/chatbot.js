...
function Chatbot() {
  ...
  const [isPromptTrayOpen, setIsPromptTrayOpen] = useState(true);
  const [isBodyMapExpanded, setIsBodyMapExpanded] = useState(false);
  ...
  const handleExpandedMuscleSelect = (muscle) => {
    setIsBodyMapExpanded(false);
  ...
  const submitMessage = async (messageText, options = {}) => {
    ...
    try {
      ...
      if (requestError?.response?.status === 429 && requestError?.response?.data?.upgradeRequired) {
        const upgradeMessage = requestError.response.data.message || "Upgrade to Shaky Pro to keep using AI coaching.";

        if (requestError.response.data.aiQuota) {
          mergeStoredUser({ aiQuota: requestError.response.data.aiQuota });
          setAiQuota(requestError.response.data.aiQuota);
        }
        ...
        setMessages((prev) => [
          ...
          {
            role: "assistant",
            text: `${upgradeMessage} Redirecting you to Pro plans now.`
          }
        ]);
        navigate(requestError.response.data.redirectTo || "/plans");
        return false;
      }
  ...
  return (
    <aside className="chatbot-panel fixed bottom-4 right-4 z-50 flex max-h-[calc(100vh-2rem)] w-[min(24rem,calc(100vw-2rem))] flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl">
      <div className="chatbot-panel__header rounded-t-3xl bg-slate-950 px-4 py-3 text-white">
        <div className="flex items-start justify-between gap-3">
          <div className="flex min-w-0 items-center gap-3">
        ...
        <p className="chatbot-panel__description mt-3 text-sm text-slate-300">Signed in as {currentUser.username}. Ask Shaky about workouts, diet plans, recovery, routines, measurements, or how to use XTracker.</p>
        <p className="chatbot-panel__status-pill mt-2 inline-flex rounded-2xl bg-cyan-300 px-3 py-1 text-[11px] font-black uppercase tracking-[0.16em] text-slate-950">
      ...
      <div aria-live="polite" className="chatbot-panel__messages flex-1 space-y-3 overflow-y-auto px-4 py-4">
        {messages.length === 0 && (
      ...
      <div className="chatbot-panel__tray border-t border-slate-200 bg-slate-50 px-4 py-3">
        <button
          type="button"
          onClick={() => setIsPromptTrayOpen((isOpen) => !isOpen)}
          className="flex w-full items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-left text-xs font-black uppercase tracking-[0.14em] text-slate-700 shadow-sm transition hover:border-cyan-300 hover:text-cyan-700 focus:outline-none focus:ring-4 focus:ring-cyan-100"
          aria-expanded={isPromptTrayOpen}
        >
          <span>Shaky prompts</span>
          <span className="text-cyan-700">{isPromptTrayOpen ? "Hide" : "Show"}</span>
        </button>

        {isPromptTrayOpen && (
          <div className="chatbot-panel__tray-content mt-3 overflow-y-auto pr-1">
            <div className="flex flex-wrap gap-2">
              ...
              {isMusclePickerOpen && (
                <button
                  type="button"
                  onClick={() => setIsBodyMapExpanded(true)}
                  disabled={isSending}
                  className="rounded-2xl border border-cyan-200 bg-white px-3 py-2 text-xs font-black uppercase tracking-[0.12em] text-cyan-800 transition hover:border-cyan-400 hover:bg-cyan-50 disabled:cursor-not-allowed disabled:opacity-60"
                >
                  Expand body map
                </button>
              )}
            ...
            {isMusclePickerOpen && (
              <div className="chatbot-panel__body-map-compact">
                <BodyMusclePicker
      ...
      {isBodyMapExpanded && (
        <div className="fixed inset-0 z-[90] flex items-end justify-center bg-slate-950/70 p-3 sm:items-center sm:p-6">
          <div className="chatbot-body-map-modal flex max-h-[calc(100dvh-2rem)] w-full max-w-3xl flex-col overflow-hidden rounded-3xl bg-white shadow-2xl">
            <div className="flex items-center justify-between gap-3 border-b border-slate-200 bg-slate-950 px-4 py-3 text-white">
              <div className="min-w-0">
                <p className="text-xs font-black uppercase tracking-[0.18em] text-cyan-300">Shaky body map</p>
                <h3 className="mt-1 text-lg font-black">Pick a muscle group</h3>
              </div>
              <button
                type="button"
                onClick={() => setIsBodyMapExpanded(false)}
                className="rounded-2xl border border-white/15 px-3 py-2 text-xs font-black uppercase tracking-[0.15em] text-slate-200 transition hover:bg-white/10 hover:text-white"
              >
                Close
              </button>
            </div>
            <div className="chatbot-body-map-modal__body overflow-y-auto p-3 sm:p-4">
              <BodyMusclePicker
                selectedMuscle={selectedMuscle}
                onSelectMuscle={handleExpandedMuscleSelect}
                disabled={isSending}
                expanded
              />
            </div>
      ...
      <form onSubmit={handleSubmit} className="chatbot-panel__form flex gap-2 border-t border-slate-200 p-3">
        <input
          ...
          onFocus={() => setIsPromptTrayOpen(true)}
